...
import type { XUser, XFollower, XMention, XRetweet, XTweet } from "../types/rapidapi";
...
export async function syncFollowers(
  ...
  try {
    ...
    const previousFollowers =
      (await cache.get<XFollower[]>(cacheKey)) || [];
...
export async function syncTimeline(
  ...
  try {
    ...
    await cache.set(cacheKey, { tweets, lastPostTime }, SYNC_INTERVALS.timeline * 60);
    ...
    console.log(`[Sync] Timeline: ${tweets.length} tweets, last post ${lastPostTime}`);
 ...
 */
export async function getLastMentionId(userId: string): Promise<string | undefined> {
  return cache.get<string>(cache.cacheKey(userId, "mentions", "last_id"));
}

/**
 * Get last post time from cache
 */
export async function getLastPostTime(userId: string): Promise<number | undefined> {
  return cache.get<number>(cache.cacheKey(userId, "timeline", "last_post_time"));
}
...
export async function getSyncStatus(userId: string): Promise<{
  ...
  return {
    lastMentionAt: now - (await cache.ttl(cache.cacheKey(userId, "mentions"))) * 1000,
    lastFollowerSyncAt: now - (await cache.ttl(cache.cacheKey(userId, "followers"))) * 1000,
    lastTimelineSyncAt: now - (await cache.ttl(cache.cacheKey(userId, "timeline"))) * 1000,
