...
import type { XMention, XFollower, XTweet } from "../types/rapidapi";

// Try to import simdjson, fallback to JSON.parse if not available
let simdjson: { parse: (json: string) => any } | null = null;
try {
    ...
    for (const instruction of instructions) {
      ...
      for (const entry of entries) {
        ...
        mentions.push({
          ...
          hashtags: legacy.entities?.hashtags?.map((h: any) => String(h.text)) || [],
          mentions: legacy.entities?.user_mentions?.map((m: any) => ({
...
export function extractFollowersSimd(jsonString: string): XFollower[] {
  try {
    ...
    const instructions = doc.data?.user?.result?.timeline?.timeline?.instructions;
...
export function extractRetweetsSimd(jsonString: string, tweetId: string) {
  try {
    ...
    const totalRetweets = Number(doc.data?.source_tweet?.legacy?.retweet_count) || 0;
 ...
 */
export function benchmarkParsing(jsonString: string, iterations: number = 100): {
  simdjsonTime: number;
  standardTime: number;
  speedup: number;
} {
