...
import type { TriggerData } from "../actions/types";
	...
	for (const action of workflow.actions) {
		...
		if (action.condition) {
			const conditionMet = evaluateCondition(
				action.condition as unknown as Condition,
		...
		if (action.delay && action.delay > 0) {
			await new Promise((resolve) =>
				setTimeout(resolve, Math.min(action.delay ?? 0, 5000)),
			);
		}
...
export function createActionContext(
	...
	ActionContext,
	"workflowId" | "runId" | "triggerData" | "previousResults"
> {
	return {
		userId,
		xUserId,
