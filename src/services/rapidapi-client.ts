...
async function rapidApiRequest<T>(
	...
	if (remaining) {
		rateLimitStatus.remaining = Number.parseInt(remaining, 10);
	}
	if (reset) {
		rateLimitStatus.resetTime = Number.parseInt(reset, 10) * 1000;
	}
...
export function canMakeRequest(): boolean {
	return (
		rateLimitStatus.remaining > 0 || Date.now() >= rateLimitStatus.resetTime
	);
}
