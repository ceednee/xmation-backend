...
describe("Trigger Engine", () => {
  describe("evaluateTrigger", () => {
    ...
    it("should return false for disabled trigger", async () => {
      ...
      const context = {
        userId: "user_123",
        mentions: [{ id: "m1", text: "hello", authorId: "u1", authorUsername: "@alice", createdAt: Date.now() }],
      };
  ...
  describe("evaluateWorkflowTriggers", () => {
    it("should evaluate all triggers in a workflow", async () => {
      ...
      const context = {
        userId: "user_123",
        mentions: [{ id: "m1", text: "hi", authorId: "u1", authorUsername: "@alice", createdAt: Date.now() }],
        dms: [],
      };
    ...
    it("should trigger if ANY condition is met (OR logic)", async () => {
      ...
      const context = {
        userId: "user_123",
        mentions: [],
        dms: [{ id: "dm1", senderId: "u1", senderUsername: "@bob", text: "hi", createdAt: Date.now() }],
      };
      ...
      const workflows: Workflow[] = [
        {
          ...
          currentVersionId: "",
          isDryRun: false,
          triggers: [{ id: "tr_1", type: "NEW_MENTION", config: {}, enabled: true }],
          actions: [],
          createdAt: Date.now(),
          updatedAt: Date.now(),
        },
