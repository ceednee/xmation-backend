...
describe("Auth Middleware", () => {
	describe("requireAuth", () => {
		it("should reject requests without authorization header", async () => {
			const middleware = ({ request, set }: Context) => {
			...
			const app = new Elysia().get("/protected", () => "secret", {
		...
		it("should allow requests with valid Bearer token", async () => {
			...
			const app = new Elysia().get("/protected", () => ({ data: "secret" }), {
				beforeHandle: middleware,
			});
	...
	describe("requireXConnection", () => {
		it("should reject requests without X OAuth connection", async () => {
			...
			const app = new Elysia().get("/workflows", () => "workflows", {
				beforeHandle: middleware,
			});
	...
	describe("rateLimit", () => {
		it("should track request counts per user", async () => {
			...
			const app = new Elysia().get("/api", () => ({ success: true }), {
				beforeHandle: middleware,
			});
