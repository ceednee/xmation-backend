...
describe("Workflows API", () => {
    ...
    it("should validate required fields", async () => {
      const app = new Elysia()
        .post("/workflows", async ({ body, set }: any) => {
          if (!body.name) {
            set.status = 400;
            return { success: false, error: { code: "NAME_REQUIRED", message: "Workflow name is required" } };
          }
          return { success: true };
        });
  ...
  describe("GET /workflows - List Workflows", () => {
    ...
    it("should filter workflows by status", async () => {
      ...
      const response = await app.handle(new Request("http://localhost/workflows?status=active"));
      const body = await response.json();
  ...
  describe("GET /workflows/:id - Get Workflow", () => {
    it("should return single workflow by ID", async () => {
      const workflow = { _id: "wf_123", name: "Test Workflow", status: "active" };
      
      const app = new Elysia()
        .get("/workflows/:id", ({ params, set }: any) => {
      ...
      const response = await app.handle(new Request("http://localhost/workflows/wf_123"));
    ...
    it("should return 404 for non-existent workflow", async () => {
      const app = new Elysia()
        .get("/workflows/:id", ({ set }: any) => {
          set.status = 404;
          return { success: false, error: { code: "NOT_FOUND", message: "Workflow not found" } };
        });

      const response = await app.handle(new Request("http://localhost/workflows/invalid_id"));
  ...
  describe("PATCH /workflows/:id - Update Workflow", () => {
    it("should update workflow fields", async () => {
      const workflow = { _id: "wf_123", name: "Old Name", description: "Old desc", status: "draft" };
      
      const app = new Elysia()
        .patch("/workflows/:id", async ({ params, body }: any) => {
          if (params.id === workflow._id) {
    ...
    it("should not allow updating active workflow triggers/actions", async () => {
      const workflow = { _id: "wf_123", name: "Test", status: "active", triggers: [], actions: [] };
      
      const app = new Elysia()
        .patch("/workflows/:id", ({ body, set }: any) => {
          if (workflow.status === "active" && (body.triggers || body.actions)) {
            set.status = 400;
            return { 
              success: false, 
              error: { code: "WORKFLOW_ACTIVE", message: "Cannot modify active workflow. Pause first." } 
            };
          }
          return { success: true, data: workflow };
        });
  ...
  describe("DELETE /workflows/:id - Delete Workflow", () => {
    it("should delete workflow by ID", async () => {
      ...
      const app = new Elysia()
        .delete("/workflows/:id", ({ params }: any) => {
  ...
  describe("POST /workflows/:id/activate - Activate Workflow", () => {
    it("should activate draft workflow", async () => {
      ...
      const app = new Elysia()
        .post("/workflows/:id/activate", ({ params, set }: any) => {
      ...
      const response = await app.handle(
        new Request("http://localhost/workflows/wf_123/activate", { method: "POST" })
      );
    ...
    it("should not activate workflow without triggers", async () => {
      const workflow = { _id: "wf_123", name: "Test", status: "draft", triggers: [], actions: [] };
      
      const app = new Elysia()
        .post("/workflows/:id/activate", ({ set }: any) => {
          if (workflow.triggers.length === 0) {
            set.status = 400;
            return { success: false, error: { code: "NO_TRIGGERS", message: "Workflow must have at least one trigger" } };
          }
          return { success: true };
        });
  ...
  describe("POST /workflows/:id/pause - Pause Workflow", () => {
    it("should pause active workflow", async () => {
      ...
      const app = new Elysia()
        .post("/workflows/:id/pause", ({ set }: any) => {
          if (workflow.status !== "active") {
            set.status = 400;
      ...
      const response = await app.handle(
        new Request("http://localhost/workflows/wf_123/pause", { method: "POST" })
      );
  ...
  describe("POST /workflows/:id/test - Dry Run Test", () => {
    it("should execute workflow in dry-run mode", async () => {
      const app = new Elysia()
        .post("/workflows/:id/test", async ({ body }: any) => {
          return {
            success: true,
            data: {
              mode: "dry_run",
              wouldExecute: true,
              actions: [
                { action: "WELCOME_DM", wouldSend: true, recipient: "@testuser" },
              ],
              logs: ["Trigger: NEW_FOLLOWER detected", "Action: WELCOME_DM would be sent"],
            },
          };
        });
