...
async function mockConvexCreateSession(data: SessionData) {
	return {
...
async function mockHandleOAuthCallback(data: OAuthCallbackData) {
	return {
		success: true,
		userId: "user_new",
...
async function mockStoreXTokensSecurely(userId: string, tokens: XTokens) {
	return {
...
async function mockVerifyConvexSession(
	token: string,
...
interface XConnectionResult {
	connected: boolean;
	error?: string;
}

async function mockCheckXConnection(
	userId: string,
): Promise<XConnectionResult> {
	if (userId === "user_no_x") {
...
async function mockVerifyWithAutoRefresh(
	userId: string,
