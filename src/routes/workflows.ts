...
import type { Workflow, TriggerConfig, ActionConfig } from "../types";
...
const updateWorkflowSchema = t.Partial(t.Object({
  name: t.String({ minLength: 1, maxLength: 100 }),
...
export const workflowRoutes = new Elysia({ prefix: "/workflows" })
  ...
  .post("/", async ({ body, request }: any) => {
    ...
    const workflow: Workflow = {
      ...
      triggers: body.triggers.map((t: any, i: number) => ({
        id: `tr_${i}`,
        type: t.type,
        config: t.config || {},
        enabled: true,
  ...
  .patch("/:id", async ({ params, body, request, set }: any) => {
    const workflow = workflowsStore.get(params.id);
  ...
  .post("/:id/activate", ({ params, set }: any) => {
    const workflow = workflowsStore.get(params.id);
    ...
    if (workflow.status === "active") {
      ...
      return {
        success: false,
        error: { code: "ALREADY_ACTIVE", message: "Workflow is already active" },
      };
    }
  ...
  .post("/:id/test", ({ params, body, set }: any) => {
    const workflow = workflowsStore.get(params.id);
